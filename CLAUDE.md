...
| Supabase | DB・認証 | ✅ 完了 |
| Claude API (Anthropic) | AI月次分析 | ✅ 完了 |
| Stripe | 決済・サブスク | ⬜ 未実装 |
| Vercel | ホスティング | ✅ 完了 |
...
- コメント（日記テキスト）入力・保存
- ページを開くと今日の記録を自動ロード
- 月次AI分析（`/api/analyze`）— Claude Haiku呼び出し、当月の全記録を分析して表示
...
### 1. Anthropic APIキーを設定して動作確認 ★最優先
コードは完成済み。あとはAPIキーをセットするだけ。

**junpeiさんがやること（2ステップ）：**
1. https://console.anthropic.com/ にログイン → 「API Keys」→ 「Create Key」でキーをコピー
2. `.env.local` の `ANTHROPIC_API_KEY=` の後ろに貼り付け
3. Vercel → Settings → Environment Variables → `ANTHROPIC_API_KEY` を追加 → Redeploy

設定後、ログイン画面で「今月の物語を読み解く ✦」ボタンが押せるようになる。
...
- [ ] **Anthropic APIキーを取得して `.env.local` とVercelに設定する**（https://console.anthropic.com/）
...
| 変数名 | 用途 | 設定場所 |
|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase接続 | .env.local ✅ / Vercel ✅ |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase認証 | .env.local ✅ / Vercel ✅ |
| `ANTHROPIC_API_KEY` | Claude API（月次分析） | .env.local ⬜ / Vercel ⬜ |
